# Diagnostic isolation: teleport the trapped UNIs UP to the surface (room to place a table) so we can tell
# whether the WS-A craft fix produces tools when not bedrock-trapped. Standalone RCON.
defmodule R do
...
if aid == 1 do
  for u <- ["UNI-0-1", "UNI-1-1", "UNI-1-2", "UNI-1-3", "UNI-2-1", "UNI-3-1"] do
    R.cmd(sock, "tp #{u} ~ 80 ~")
  end
end
:gen_tcp.close(sock)
